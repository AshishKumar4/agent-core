...
describe("Environment records", () => {
    ...
    test(
        ...
        { tags: "p1" },
        () => {
            const records: readonly RecordCodecCase[] = [
                recordCodecCase("Environment", environment, Environment.encode, Environment.decode),
                recordCodecCase(
                    "Environment revision",
                    revisionRecord,
                    EnvironmentRevisionRecord.encode,
                ...
                recordCodecCase("Port exposure", exposure, PortExposure.encode, PortExposure.decode)
            ];

            for (const record of records) {
                const envelope = decodeRecordEnvelope(record.bytes);

                expect(envelope.version).toEqual({ major: 1, minor: 0 });
                expect(record.roundTrip()).toEqual(record.bytes);
                expect(record.fixtureIsFrozen()).toBe(true);
            }
        }
    );
    ...
    test("rejects an unknown major for every Environment record", { tags: "p2" }, () => {
        ...
        for (const record of records) {
            ...
            expect(() => record.decode(unknownMajor)).toThrow(
                new AgentCoreError(
                    "codec.unknown-major",
    ...
    test("requires branded values for every record and capability field", { tags: "p1" }, () => {
        ...
        // @ts-expect-error The runtime contract must reject an object that is not a ProviderId.
        const invalidProviderId: ProviderId = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a string.
        const invalidString: string = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a ContentRef.
        const invalidContent: ContentRef = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not an EnvironmentId.
        const invalidEnvironmentId: EnvironmentId = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a Revision.
        const invalidRevision: Revision = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a ProviderDescriptor.
        const invalidProvider: ProviderDescriptor = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not an EnvironmentSessionId.
        const invalidSessionId: EnvironmentSessionId = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a session capability.
        const invalidSessionCapability: EnvironmentSessionCapability = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a SecretRef.
        const invalidSecret: SecretRef = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a session state.
        const invalidSessionState: EnvironmentSessionState = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a snapshot ID.
        const invalidSnapshotId: EnvironmentSnapshotId = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not a snapshot state.
        const invalidSnapshotState: EnvironmentSnapshotState = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not an exposure ID.
        const invalidExposureId: PortExposureId = malformed;
        // @ts-expect-error The runtime contract must reject an object that is not an exposure state.
        const invalidExposureState: PortExposureState = malformed;
        const credential = new SecretRef("vault", "credential-provider", "strict-shape");
        const actions = [
            () => new ProviderDescriptor(invalidProviderId, "1", configuration),
            () => new ProviderDescriptor(provider.id, invalidString, configuration),
            () => new ProviderDescriptor(provider.id, "1", invalidContent),
            () => new Environment(invalidEnvironmentId, Revision.initial(), 0, Revision.initial()),
            () => new Environment(environmentId, invalidRevision, 0, Revision.initial()),
            () => new Environment(environmentId, Revision.initial(), 0, invalidRevision),
            () =>
                new EnvironmentRevisionRecord(
            ...
            () => new EnvironmentRevisionRecord(environmentId, invalidRevision, 0, provider),
            () =>
                new EnvironmentRevisionRecord(
                    environmentId,
                    Revision.initial(),
            ...
            () => new EnvironmentSessionCapability(environmentId, sessionId, invalidRevision, 0),
            () => new EnvironmentCredentialProxyCapability(invalidSessionCapability, 0, credential),
            () => new EnvironmentCredentialProxyCapability(session.capability, 0, invalidSecret),
            () =>
