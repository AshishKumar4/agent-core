...
describe("Invocation evidence records", () => {
    test(
        "guards every Approval terminal transition and round-trips revisions",
        { tags: "p0" },
        () => {
    ...
    test(
        "round-trips executor and system claims and enforces recovery ownership",
        { tags: "p0" },
        () => {
    ...
    test(
        "round-trips every Receipt variant and rejects illegal result lineage",
        { tags: "p1" },
        () => {
            ...
            expect(
                decodedSucceeded instanceof AttemptReceipt
                    ? decodedSucceeded.result?.value
                    : undefined
    ...
    test(
        "[C13-PREPARED-ROUTED-PROJECTION] covers every BatchOutcome precedence and terminal projection",
        { tags: "p1" },
        () => {
    ...
    test(
        "consumes an Approval only from the approved state and exactly at or after approval",
        { tags: "p0" },
        () => {
    ...
    test(
        "guards every Approval decision against its request and expiry boundaries",
        { tags: "p1" },
        () => {
    ...
    test(
        "enforces ItemClaim identity, expiry, and recovery ownership precisely",
        { tags: "p0" },
        () => {
    ...
    test(
        "rejects EffectAttempt identifier, ordinal, and start-time violations",
        { tags: "p1" },
        () => {
            class DerivedInvocationId extends InvocationId {}
            class DerivedItemClaimId extends ItemClaimId {}
    ...
    test(
        "rejects Receipt identifier violations and misimplemented variants",
        { tags: "p1" },
        () => {
    ...
    test(
        "requires InvocationContinuation identifiers, canonical keys, and a valid admission time",
        { tags: "p1" },
        () => {
            class DerivedApprovalId extends ApprovalId {}
            class DerivedEffectAttemptId extends EffectAttemptId {}
    ...
    test(
        "copies each Approval state into exactly the fields that state carries",
        { tags: "p1" },
        () => {
    ...
    test(
        "rejects a pre-effect Receipt outcome before reading the rest of the payload",
        { tags: "p2" },
        () => {
    ...
    test(
        "derives a batch outcome only from one complete Receipt slot per item",
        { tags: "p1" },
        () => {
