...
describe("CanonicalBatchInvocationPort", () => {
    ...
    test(
        "[C13-CLAIM-INITIAL-ATOMIC] claims before async permit issuance and returns one output only for success",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[C13-EFFECT-IDEMPOTENCY] reuses a durable claim after permit-call crash and never repeats the effect",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[C13-CLAIM-RECOVERY-NEW-OWNER] rejects empty, inexact, and substituted canonical batch preparation before claiming",
        { tags: "p0" },
        async () => {
            ...
            await expect(
                inexact.port.invoke({
                    ...one,
                    request: { ...one.request, cardinality: { kind: "batch", itemCount: 2 } }
                })
            ).rejects.toThrow(/nonempty exact payload shape/);
    ...
    test(
        "[C13-RECEIPT-PRE-EFFECT] persists the exact Invocation-to-deniedPreEffect Receipt edge",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "rejects an existing PreparedInvocation without its exact Invocation audit root",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "atomically rejects an EffectAttempt whose Invocation audit cause is substituted",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[P11-DEVICE-CONSENT-ABSENT] records final target-admission denial with no EffectAttempt",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[P11-DEVICE-CONSENT-REVOCATION] linearizes a revocation after permit issue but before final target admission",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[P11-DEVICE-CONSENT-ADMITTED] does not let post-admission revocation cancel an admitted effect",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[C13-CLAIM-RECOVERY-SAME-ORDINAL] recovers an expired permit-stage claim under a new worker at the same ordinal",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "[C13-EFFECT-RECONCILIATION] terminalizes a crash-recovered admitted attempt without repeating the effect",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        "rejects a Receipt audit whose cause is not the exact attempt audit",
        { tags: "p0" },
        async () => {
            const harness = new Harness(false);
    ...
    test(
        ...
        async () => {
            ...
            expect(durable.attempts[0]?.claim.value).toBe(`claim:${invocation.value}:0:recovered`);
        }
    );
    ...
    test(
        "rejects inexact input and interception counts independently",
        { tags: "p1" },
        async () => {
            const inputs = new Harness(false);
            const value = request(
                new InvocationId("inexact-inputs"),
                [{ value: 1 }],
                () => undefined
            );
            await expect(
    ...
    test(
        "rejects a prepared invocation whose operation binding drifts",
        { tags: "p1" },
        async () => {
            ...
            await expect(
                kind.port.invoke(
                    request(
                        new InvocationId("prepared-kind-drift"),
                        [{ value: 1 }],
                        () => undefined
                    )
                )
            ...
            const other = new CanonicalBatchPreparation<string>(
                false,
                new FacetRef("workspace:other")
            );
            ...
            await expect(
                target.port.invoke(
                    request(
                        new InvocationId("prepared-target-drift"),
                        [{ value: 1 }],
                        () => undefined
                    )
                )
            ...
            impact.preparation.override = () =>
                impactDriftedPrepared(impactInvocation, [{ value: 1 }]);
            await expect(
    ...
    test(
        "rejects prepared items that drift from the canonical inputs",
        { tags: "p1" },
        async () => {
            ...
            await expect(
                count.port.invoke(
                    request(
                        new InvocationId("prepared-count-drift"),
                        [{ value: 1 }],
                        () => undefined
                    )
                )
            ...
            await expect(
                single.port.invoke(
                    request(
                        new InvocationId("prepared-item-drift"),
                        [{ value: 1 }],
                        () => undefined
                    )
                )
            ...
            partial.preparation.override = (bound) =>
                partial.preparation.create(bound.invocation, [
                    bound.request.inputs[0],
                    { value: 999 }
                ]);
            await expect(
                partial.port.invoke(
