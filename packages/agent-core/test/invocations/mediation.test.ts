...
describe("W6 operation mediation integration", () => {
    test(
        "memory audit evidence relations survive restart and remain actor-owned",
        { tags: "p1" },
        () => {
            ...
            corrupt.auditByEvidence.set(
                corrupt.auditByEvidence.keys().next().value!,
                "missing-audit"
            );
    ...
    test(
        ...
        () => {
            ...
            expect(
                InvocationPublicationOutbox.decode(InvocationPublicationOutbox.encode(publication))
                    .id.value
    ...
    test(
        "[C13-PREPARED-REPLAY-POST] [invocation.mediated-replay] [invocation-replay-persistence] reserves before interception and reuses durable before/after presentation",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[C13-PREPARED-REPLAY-PRE] commits raw replay identity before mutating interception and rejects conflicts first",
        { tags: "p0" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
        ...
        async () => {
            ...
            expect(
                transactions.transact(
                    (transaction) =>
                        persistence.publication(transaction, publication.id)?.state.kind
                )
            ).toBe("published");
            await drainer.flush();
    ...
    test(
        "persists independent sink acknowledgements and never republishes an acknowledged sink",
        { tags: "p0" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
    ...
    test(
        "[C13-ADV-APPROVAL-REPLAY] rejects noncanonical replay scopes, attempted direct contexts, and unprepared invocation",
        { tags: "p0" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
            ...
            expect(() =>
                port.directContext(
                    new OperationRequestKey("direct"),
                    0,
                    { kind: "single" },
                    "permit"
                )
    ...
    test(
        "keeps a reservation recoverable when before interception returns malformed cardinality",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[C13-BATCH-OUTCOME-TERMINAL] persists terminal batch evidence and fails closed on every replay",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[C13-PREPARED-APPROVAL-BINDING] binds presentation to exact invocation evidence, item count, and persisted outputs",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[C13-ADV-NONHOMOGENEOUS-BATCH] enforces replay phase immutability and nonempty exact batch shape",
        { tags: "p0" },
        () => {
            ...
            expect(() => reserved.recordEffect(0, {}, new ReceiptId("early"))).toThrow(
                /preparation/
            );
            ...
            expect(() =>
                reserved.prepare(
                    new InvocationId("wrong-trace"),
                    [{}],
                    [[trace("operation.after")]]
                )
    ...
    test(
        "[C13-BATCH-OUTCOME-COMPLETE] maps profile receipt mode and terminal outcomes without leaking batch shapes",
        { tags: "p1" },
        async () => {
            ...
            await expect(receiptPort.invoke(request)).resolves.toEqual({
                kind: "receipt",
                receipt
            });
            ...
            for (const terminal of outcomes) {
                ...
                await expect(
                    port.invoke({ ...request, resultMode: "output" })
    ...
    test(
        "[C13-PREPARED-NO-TURN-OWNER] rejects substituted continuation identifiers, owner kinds, actor kinds, and indexes",
        { tags: "p0" },
        () => {
            ...
            expect(() => create(new SubstitutedInvocationId("substituted"))).toThrow(
                /exact context/
            );
...
describe("W6 replay operation invocation port", () => {
    ...
    test(
        "names the changed bound intent of a reused OperationRequestKey",
        { tags: "p2" },
        async () => {
    ...
    test(
        "returns the durable preparation when a concurrent actor prepares first",
        { tags: "p1" },
        async () => {
            const { port, transactions, persistence, invocation } =
                replayHarness("concurrent-prepare");
    ...
    test(
        "fails when the reservation disappears before preparation commits",
        { tags: "p1" },
        async () => {
    ...
    test(
        "names the before phase that changed either item cardinality",
        { tags: "p2" },
        async () => {
    ...
    test(
        "authenticates every replay binding component before invocation",
        { tags: "p0" },
        async () => {
    ...
    test(
        "resumes a partially recorded batch without repeating persisted effects",
        { tags: "p0" },
        async () => {
            ...
            amendReplay(
                ...
                "partial-batch",
                request.requestKey.value,
                (record) => [record.recordEffect(0, { effect: 0 }, new ReceiptId("receipt-0"))]
            );
            const result = await port.invoke({
                ...request,
                inputs: prepared.kind === "new" ? prepared.preparation.inputs : [],
    ...
    test(
        "rejects canonical batch replays that change persisted evidence",
        { tags: "p0" },
        async () => {
    ...
    test(
        "fails when the replay reservation disappears during mediation",
        { tags: "p1" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
    ...
    test(
        "rejects mixed batch outcomes while persisting their evidence",
        { tags: "p0" },
        async () => {
    ...
    test("replays a completed batch with its full presented output", { tags: "p0" }, async () => {
        ...
        const { port, transactions } = replayHarness(
            "batch-replay",
            new SuccessfulBatch(invocation)
        );
    ...
    test(
        "accepts canonical batch replays that repeat persisted terminal evidence",
        { tags: "p0" },
        async () => {
            ...
            amendReplay(
                ...
                "terminal-consistency",
                request.requestKey.value,
                (record) => [record.recordTerminal(0, new ReceiptId("receipt-0"))]
            );
            await expect(
                port.invoke({
                    ...request,
    ...
    test(
        "fails closed when a complete replay lacks its Invocation evidence",
        { tags: "p1" },
        async () => {
...
describe("W6 invocation publication outbox", () => {
    test("rejects revisions that disagree with acknowledgement state", { tags: "p1" }, () => {
        ...
        expect(
            () => new InvocationPublicationOutbox(base, { kind: "pending" }, new Revision(1))
    ...
    test("orders publication lineage through follows", { tags: "p0" }, () => {
        ...
        const other = InvocationPublicationOutbox.pending(
            observation("follows-other")
    ...
    test(
        "round-trips acknowledgement states and rejects corrupted payloads",
        { tags: "p1" },
        () => {
            const base = observation("publication-codec");
            const commitOnly = InvocationPublicationOutbox.pending(base).commitAppended(
                new Date(7)
            );
            const bytes = InvocationPublicationOutbox.encode(commitOnly);
            expect(
                InvocationPublicationOutbox.encode(InvocationPublicationOutbox.decode(bytes))
            ).toEqual(bytes);
            expect(() =>
...
describe("W6 invocation publication drainer", () => {
    test(
        "drains commit-first acknowledgements without re-appending the commit",
        { tags: "p0" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
    ...
    test("tolerates sinks that acknowledge their own publication", { tags: "p1" }, async () => {
        ...
        expect(
            ...
            kind: "published",
            eventPublishedAt: new Date(5),
            commitAppendedAt: new Date(6)
        });
    });
    ...
    test(
        "surfaces publications that disappear during acknowledgement",
        { tags: "p1" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
    ...
    test(
        "tolerates commit acknowledgements that race ahead during draining",
        { tags: "p0" },
        async () => {
            const transactions = new MemoryTransactions();
            const persistence = new MemoryInvocationMediationPersistence();
...
describe("W6 mediation memory persistence", () => {
    ...
    test("guards replay append lineage", { tags: "p0" }, () => {
        ...
        expect(() => persistence.appendReplay(rewound, prepared)).toThrow(/Replay revision exists/);
    });
    ...
    test(
        "rejects replay appends whose lineage skips or lacks the reservation",
        { tags: "p0" },
        () => {
            const persistence = new MemoryInvocationMediationPersistence();
            const reserved = MediatedReplayRecord.reserve(
                replayReservation("memory-lineage-clauses")
            );
            const prepared = reserved.prepare(
    ...
    test(
        "rejects audit evidence projections misfiled under a different record id",
        { tags: "p1" },
        () => {
            const persistence = new MemoryInvocationMediationPersistence();
            const state = createInvocationMediationMemoryState();
...
describe("W6 profile mediation port", () => {
    test(
        "dispatches exactly one canonical item with empty interceptions",
        { tags: "p1" },
        async () => {
                ...
                { invocation: () => invocation },
                {
                    invoke: async (batchRequest) => {
                        ...
                        return {
                            invocation,
                            items: [
                                { kind: "succeeded", itemIndex: 0, receipt, output: { value: 2 } }
                            ]
                        };
                    }
                }
    ...
    test(
        "rejects a sparse canonical batch item from profile mediation",
        { tags: "p1" },
        async () => {
