...
import { PrincipalId, TenantId, WorkspaceId } from "../../src/identity";
...
describe("Single-tenant policy assembly", () => {
    test(
        "[P11-SINGLE-TENANT-POLICY] admits the initial Tenant and denies creating a second Tenant",
        { tags: "p0" },
        () => {
            ...
            expect(() => assembleSingleTenantPolicy(control, policyInit)).toThrow(
    ...
    test(
        "[P11-SINGLE-TENANT-PRINCIPAL] persists exactly the one anchored Principal",
        { tags: "p1" },
        () => {
    ...
    test(
        "[P11-SINGLE-TENANT-TENANT] persists exactly the one personal Tenant",
        { tags: "p1" },
        () => {
            const { control } = createAssembly();
    ...
    test(
        "[P11-SINGLE-TENANT-OWNER] materializes an active owner Membership for the Principal",
        { tags: "p1" },
        () => {
            const { assembly, control } = createAssembly();
    ...
    test(
        "[P11-SINGLE-TENANT-RECORDS] restores ordinary Grant and Binding records from durable snapshots",
        { tags: "p1" },
        () => {
            const { assembly, control } = createAssembly();
            ...
            expect(
                restartedControl
                    .binding(assembly.binding.key)
                    ?.grantId.equals(assembly.binding.grantId)
            ).toBe(true);
        }
    );

    test(
        "[P11-SINGLE-TENANT-PROMOTION] changes only Tenant multiplicity policy",
        { tags: "p1" },
        () => {
            const { assembly, control } = createAssembly();
    ...
    test(
        "[P11-SINGLE-TENANT-ASSEMBLY] assembles a personal assistant from ordinary policy records",
        { tags: "p1" },
        () => {
...
function createAssembly() {
    ...
    const assembly = assembleSingleTenantPolicy(control, policyInit);
    return { assembly, control };
}
