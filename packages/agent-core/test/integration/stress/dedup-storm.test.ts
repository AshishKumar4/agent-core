 ...
 */
async function runStorm(harness: CounterFixture, load: StormLoad, seed: string): Promise<Storm> {
    ...
    describe(title, () => {
        ...
        test(
            ...
            async () => {
                ...
                expect(after.writes).toHaveLength(total + load.replayRounds * storm.keys.length);
