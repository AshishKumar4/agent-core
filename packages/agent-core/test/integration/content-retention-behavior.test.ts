...
describe("memory content retention behavior", () => {
    test(
        "restores ownership after restart and rolls release plus collection back atomically",
        { tags: "p0" },
        async () => {
    ...
    test(
        "rejects a snapshot that retains an owner edge after cached content bytes are lost",
        { tags: "p0" },
        async () => {
            const owner = contentOwner();
