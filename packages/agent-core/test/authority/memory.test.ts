...
describe("MemoryTenantControlStore", () => {
    test(
        "[authority.grant] [authority.scope-epoch] [authority.binding] [authority.invalidation-watermark] bootstraps and restores the complete detached Tenant control snapshot",
        { tags: "p0" },
        () => {
    ...
    test(
        "keeps bootstrap atomic and eligible after a late bootstrap failure",
        { tags: "p0" },
        () => {
    ...
    test(
        "allows writes only after bootstrap and only inside an owned transaction",
        { tags: "p0" },
        () => {
            ...
            expect(() =>
                service.createPrincipal(
                    new Principal(new PrincipalId("too-early"), "user", "active")
                )
    ...
    test(
        "[C13-AUTH-EPOCH-ADVANCEMENT] memory advances durable path epochs for allow and deny changes",
        { tags: "p0" },
        () => {
            const store = bootstrappedStore();
            const service = new AuthorityMutationService(store);
            ...
            expect(
                MemoryTenantControlStore.restore(store.snapshot()).epoch(workspaceScope).epoch
            ).toBe(initial + 4);
        }
    );

    test(
        "rolls back post-bootstrap transactions and rejects asynchronous callbacks",
        { tags: "p0" },
        () => {
            const store = bootstrappedStore();
    ...
    test(
        "rejects captured-root reentry without committing an inner mutation",
        { tags: "p0" },
        () => {
            const store = bootstrappedStore();
            const before = store.snapshot();
            ...
            expect(() =>
                MemoryTenantControlStore.restore({
                    ...
                    grants: activeSnapshot.grants.map((entry) =>
                        entry.id === child.id.value
                            ? { ...entry, bytes: Grant.encode(cyclic) }
                            : entry
                    )
                })
            ).toThrow(/cycle/);
    ...
    test(
        "keeps explicitly revoked role Grants terminal across Role reconciliation",
        { tags: "p0" },
        () => {
            const store = bootstrappedStore();
            const service = new AuthorityMutationService(store);
    ...
    test(
        "restores deny authority through Team and verified guest identity closures",
        { tags: "p0" },
        () => {
            const store = bootstrappedStore();
            const service = new AuthorityMutationService(store);
            ...
            expect(() =>
                MemoryTenantControlStore.restore({
                    ...
                    identity: {
                        ...
                        records: snapshot.identity.records.filter(
                            (record) =>
                                !(record.kind === "role" && record.id === denyRole.name.value)
                        )
                    }
                })
