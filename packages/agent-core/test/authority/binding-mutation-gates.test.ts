...
function makeBinding(name = "mutation-binding"): Binding {
    return Binding.active(workspaceScope, subject, domain, new BindingName(name), grantId(), facet());
}
...
describe("authority key identity", () => {
    test("[C13-AUTH-BINDING-RESOLUTION] pins exact binding, domain, scope, and subject key formats", { tags: "p0" }, () => {
        ...
        expect(scopeKey(workspaceScope)).toBe(authorityKey("scope", [encodeScopeRef(workspaceScope)]));
        expect(subjectKey(subject)).toBe(authorityKey("subject", [encodeSubjectRef(subject)]));
    });
});

describe("Binding replacement identity", () => {
    test("[C13-AUTH-BINDING-RESOLUTION] rejects replacements that change identity or skip generation or revision", { tags: "p0" }, () => {
    ...
    test("[C13-AUTH-BINDING-RESOLUTION] refuses transitions past the final generation", { tags: "p0" }, () => {
...
describe("Binding codec field gates", () => {
    test("[C13-AUTH-BINDING-RESOLUTION] round-trips Binding data and names each malformed field exactly", { tags: "p0" }, () => {
        const binding = makeBinding();
        ...
        expectTypeError(() => Binding.fromData({ ...data, name: 3 }), "Binding name must be a string");
        expectTypeError(() => Binding.fromData({ ...data, grantId: 3 }), "Grant ID must be a string");
        expectTypeError(
    ...
    test("[C13-AUTH-BINDING-RESOLUTION] strictly decodes protection domains", { tags: "p0" }, () => {
        ...
        const frontend = decodeDomain({ kind: "frontend", label: "edge", secretPolicy: "no-secrets" });
...
describe("Binding validation request and evidence gates", () => {
    test("[C13-AUTH-PATH-EVIDENCE] accepts exact boundary values and rejects noncanonical inputs", { tags: "p0" }, () => {
        expect(new BindingValidationRequest({ ...validationInit(), workspaceFence: 0 }).workspaceFence).toBe(0);
        expectTypeError(
    ...
    test("[C13-AUTH-PATH-EVIDENCE] decodes workspace fences as non-negative safe integers only", { tags: "p0" }, () => {
        const payload = validationRequest().toData();
        expect(BindingValidationRequest.fromData({ ...payload, workspaceFence: 0 }).workspaceFence).toBe(0);
    ...
    test("[C13-AUTH-PATH-EVIDENCE] binds evidence to the exact request it validated", { tags: "p0" }, () => {
        const record = validationEvidence();
        expect(record.binds(validationRequest())).toBe(true);
        expect(
            record.binds(
                new BindingValidationRequest({ ...validationInit(), nonce: "binding-mutation-differs" })
            )
        ).toBe(false);
    });
...
describe("memory Binding store gates", () => {
    test("[C13-AUTH-BINDING-RESOLUTION] lists and snapshots Bindings in key order with detached bytes", { tags: "p0" }, () => {
    ...
    test("[C13-AUTH-BINDING-RESOLUTION] persists replacements and rejects conflicting saves exactly", { tags: "p0" }, () => {
        const store = new MemoryBindingStore(workspaceScope);
        const base = makeBinding();
        store.save(base);
        store.save(base);
    ...
    test("[C13-AUTH-BINDING-RESOLUTION] rejects malformed snapshots and records with exact codec errors", { tags: "p0" }, () => {
        ...
        const callableSnapshot = Object.assign(() => undefined, { records: [record], version: 1 });
        expectAgentError(
        ...
        expectAgentError(
            () => new MemoryBindingStore(workspaceScope, { version: 1, records: [undefined as never] }),
            "codec.invalid",
            recordMessage
        );
        expectAgentError(
...
describe("Grant identifier gates", () => {
    test("[C13-AUTH-DENY-PATH] enforces exact identifier length boundaries and names", { tags: "p0" }, () => {
        expectTypeError(
            () => GrantId.forRole("", 0),
        ...
        expectTypeError(() => new GrantId(""), "Grant ID must contain between 1 and 256 characters");
    });
});

describe("canonical argument freezing", () => {
    test("[C13-AUTH-PATH-EVIDENCE] deep-freezes canonical check arguments including nested objects", { tags: "p0" }, () => {
