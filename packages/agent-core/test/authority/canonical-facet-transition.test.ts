...
describe("coordinated FacetRef and CapabilitySpec transition", () => {
    test(
        "[C13-AUTH-ROLE-MATERIALIZATION] retains the exact W3 CapabilitySpec through Role and Grant runtime records",
        { tags: "p0" },
        () => {
    ...
    test(
        "[C13-AUTH-BINDING-RESOLUTION] retains the exact W3 FacetRef through Binding runtime records",
        { tags: "p0" },
        () => {
            ...
            expect(() => new FacetRef("workspace:mail:forged")).toThrow(
                /<facet-package-id>:<instance>/
            );
        }
    );
});
