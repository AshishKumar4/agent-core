...
describe("Scope epoch and path evidence mutation gates", () => {
    test(
        "distinguishes epochs by Scope and reports exhaustion for the exact Scope",
        { tags: "p0" },
        () => {
            expect(
                new ScopeEpoch(tenantScope, 1).equals(
                    new ScopeEpoch(ScopeRef.tenant(otherTenant), 1)
                )
            ).toBe(false);
            expect(new ScopeEpoch(tenantScope, 1).equals(new ScopeEpoch(tenantScope, 1))).toBe(
                true
            );
    ...
    test(
        "compares path evidence by length and diffs Scope changes at equal epochs",
        { tags: "p0" },
        () => {
            ...
            const swapped = new PathEpochEvidence([
                new ScopeEpoch(tenantScope, 1),
                new ScopeEpoch(
                    ScopeRef.workspace(tenantId, new WorkspaceId("epoch-gate-swapped")),
                    2
                )
            ]);
                    ...
                    new PathEpochEvidence([
                        ...
                        new ScopeEpoch(
                            ScopeRef.workspace(
                                tenantId,
                                projectId,
                                new WorkspaceId("epoch-gate-extra")
                            ),
                            1
                        )
                    ])
            ...
            expect(
                () =>
                    new PathEpochEvidence([
                        new ScopeEpoch(tenantScope, 1),
                        new ScopeEpoch(
                            ScopeRef.project(tenantId, new ProjectId("epoch-gate-mismatch")),
                            1
                        ),
                        new ScopeEpoch(projectWorkspaceScope, 1)
                    ])
...
describe("invalidation watermark mutation gates", () => {
    test(
        "joins watermarks monotonically without revising unchanged entries",
        { tags: "p0" },
        () => {
    ...
    test(
        "canonically orders delivered entries regardless of construction order",
        { tags: "p0" },
        () => {
...
describe("authority check request and evidence mutation gates", () => {
    ...
    test(
        "enforces decision-reason coherence and canonical Grant evidence exactly",
        { tags: "p0" },
        () => {
            expect(() =>
    ...
    test(
        "binds evidence to the exact request digest, Binding key, and generation",
        { tags: "p0" },
        () => {
            expect(evidence().binds(baseRequest)).toBe(true);
            expect(
                evidence({ requestDigest: Digest.sha256(Uint8Array.of(9)) }).binds(baseRequest)
            ).toBe(false);
...
describe("memory invalidation watermark store mutation gates", () => {
    test(
        "enforces owner identity, initialization, and revision continuity with exact codes",
        { tags: "p0" },
        () => {
    ...
    test(
        "rejects malformed snapshots and records with the exact codec error",
        { tags: "p0" },
        () => {
