...
describe("Self protected Run contract", () => {
    test(
        "[P11-SELF-COMPOSITION] composes mediated Facet Operations over the typed Run dependency",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-SELF-FINISH-MEMBRANE] invokes finish through mediation before the Run dependency",
        { tags: "p0" },
        async () => {
            const run = new TestRunDependency();
    ...
    test(
        "[P11-SELF-SPAWN-MEMBRANE] invokes spawn through mediation before the Run dependency",
        { tags: "p0" },
        async () => {
            const run = new TestRunDependency();
    ...
    test(
        "[P11-SELF-MEDIATION] routes all five Operations without caller-supplied leases or authority",
        { tags: "p0" },
        async () => {
            const run = new TestRunDependency();
    ...
    test(
        "internal runtime routes all five Operations to the typed Run dependency",
        { tags: "p1" },
        async () => {
            const run = new TestRunDependency();
    ...
    test(
        "[P11-SELF-AUTHORITY] denial prevents the typed Run dependency",
        { tags: "p0" },
        async () => {
            const run = new TestRunDependency();
