...
describe("Memory protected facade", () => {
    test(
        "[P11-MEMORY-PROMPT] materializes the relevant authorized recall result into the prompt",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-MEMORY-DISCOVERY] routes all Operations, filters through host-bound access, and bounds prompt contribution",
        { tags: "p1" },
        async () => {
            const content = new TestContent();
    ...
    test(
        "[P11-MEMORY-CANONICAL] denial leaves canonical memory unchanged",
        { tags: "p0" },
        async () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
...
describe("Memory backends", () => {
    ...
    test(
        "[P11-MEMORY-REBUILD] prunes only elapsed retention and keeps derived index rebuild invisible",
        { tags: "p0" },
        () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
    ...
    test(
        "[P11-MEMORY-FORGET] validates entries, duplicate IDs, limits, time, and forget authority",
        { tags: "p0" },
        () => {
            const content = new TestContent();
            const access = new TestAccess("alice");
    ...
    test(
        "bounds prompt construction and validates every configured integer",
        { tags: "p1" },
        async () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
            ...
            backend.remember({
                id: "large",
                content: content.add({ text: "too long" }),
                createdAt: 1
            });
    ...
    test(
        "names each malformed wire field and decodes forget output strictly",
        { tags: "p2" },
        () => {
            const content = new TestContent();
            ...
            expect(() =>
                recall.decodeOutput([
                    { id: 1, content: reference.value, authority: "a", createdAt: 1 }
                ])
            ).toThrow(/Memory ID must be a string/);
            expect(() =>
            ...
            expect(() =>
                recall.decodeOutput([
                    { id: "x", content: reference.value, authority: 1, createdAt: 1 }
                ])
    ...
    test(
        "raises MemoryError with the shared invalid-input code and exact detail",
        { tags: "p1" },
        () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
    ...
    test(
        "AND-matches every query term with sorted, limited, readable results",
        { tags: "p1" },
        () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
            ...
            backend.remember({
                id: "a",
                content: content.add({ text: "alpha beta" }),
                createdAt: 1
            });
            backend.remember({
                id: "c",
                content: content.add({ text: "alpha gamma" }),
                createdAt: 1
            });

            expect(backend.recall({ query: "alpha" }).map((entry) => entry.id)).toEqual([
                "a",
                "b",
                "c"
            ]);
            expect(backend.recall({ query: " " }).map((entry) => entry.id)).toEqual([
                "a",
                "b",
                "c"
            ]);
    ...
    test(
        "prunes elapsed retention in sorted order and keeps unbounded entries",
        { tags: "p1" },
        () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
    ...
    test(
        "rebuilds the derived index only on entry changes or explicit rebuild",
        { tags: "p1" },
        () => {
            const content = new TestContent();
    ...
    test(
        "clamps prompt recall to bounds and spends the exact character budget",
        { tags: "p1" },
        async () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
    ...
    test(
        "exposes an executable internal runtime for remember, recall, and forget",
        { tags: "p1" },
        async () => {
            const content = new TestContent();
            const backend = new MemoryBackend(
            ...
            await expect(
                internal
                    .operation(new OperationName("recall"))
                    ?.execute(context, { query: "internal" })
