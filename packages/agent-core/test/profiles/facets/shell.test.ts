...
describe("Shell protected facade", () => {
    test(
        "[P11-SHELL-BOUNDARY] preserves the Filesystem path.invalid code for command escape",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[P11-SHELL-HANDOFF] hands off only to an explicitly registered external command",
        { tags: "p0" },
        async () => {
    ...
    test(
        "[P11-SHELL-RUN] selects direct tier only for a bundled Turn-owned Session",
        { tags: "p0" },
        () => {
    ...
    test(
        "[P11-SHELL-COMPOSITION] executes a declared command against the Session Filesystem profile",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-SHELL-REGISTRY] resolves only explicitly registered commands",
        { tags: "p0" },
        () => {
            const registry = new ShellCommandRegistryBackend();
    ...
    test(
        "[P11-SHELL-STREAMS] supplies streaming stdin, stdout, and stderr to commands",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-SHELL-UNKNOWN] rejects unknown commands without external handoff",
        { tags: "p0" },
        async () => {
            const registry = new ShellCommandRegistryBackend();
            const shell = createShell(registry);
    ...
    test(
        "[P11-SHELL-FILESYSTEM] routes run/cancel, injects IO privately, and binds commands to exact env.fs",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-SHELL-CANCELLATION] cancel force-terminates within the configured bound and fences a non-cooperative process",
        { tags: "p0" },
        async () => {
            ...
            const cancelling = shell
                .cancel({ executionId: executionId("stubborn") })
                .then((value) => {
                    cancelSettled = true;
                    return value;
                });
    ...
    test(
        "[P11-SHELL-SINGLE-AUTHORITY] denial prevents command execution and IO acquisition",
        { tags: "p0" },
        async () => {
            let starts = 0;
            const registry = new ShellCommandRegistryBackend();
...
describe("Shell backends", () => {
    test(
        "[P11-SHELL-PARSER] tokenizes explicitly and rejects duplicate or malformed commands",
        { tags: "p1" },
        () => {
            ...
            expect(() =>
                registry.register("not valid", {
                    start: () => immediateProcess(Promise.resolve(0))
                })
    ...
    test(
        "validates termination bounds, exit-code wire values, and missing command targets",
        { tags: "p2" },
        async () => {
    ...
    test(
        "keeps execution IDs exclusive and tokenizes quote characters inside another quote",
        { tags: "p1" },
        async () => {
            ...
            const shell = createShell(registry);
            const running = shell.run({
                executionId: executionId("exclusive"),
                commandLine: "wait"
            });
    ...
    test(
        "fails closed across force, confirmation, fence, and completion failures",
        { tags: "p0" },
        async () => {
            const never = new Promise<number>(() => {});
            let fences = 0;
