...
describe("Single-tenant policy profile", () => {
    test(
        "[P11-SINGLE-TENANT-NO-OPERATIONS] exposes no Operation entry point",
        { tags: "p1" },
        () => {
    ...
    test(
        "[P11-SINGLE-TENANT-NO-EVENTS] exposes no Event or impact declaration",
        { tags: "p1" },
        () => {
    ...
    test(
        "[P11-SINGLE-TENANT-NO-MACHINERY] declares no Operations, Events, Facet runtime, or installation machinery",
        { tags: "p1" },
        async () => {
            expect(SINGLE_TENANT_OPERATIONS).toEqual([]);
            expect(SINGLE_TENANT_EVENTS).toEqual([]);
