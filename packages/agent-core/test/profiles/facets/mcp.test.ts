...
describe("MCP protected lifecycle and durable operation registration", () => {
    test(
        "[P11-MCP-INVOCATION] mediates lifecycle, discovery, and discovered calls",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-MCP-ADAPTER] [P11-MCP-TOOLS] [facet.mcp-discovery-registration] restores validated immutable operation registration without rediscovery",
        { tags: "p1" },
        async () => {
    ...
    test(
        "[P11-MCP-LIFECYCLE] denial prevents server lifecycle handlers",
        { tags: "p0" },
        async () => {
            const server = new TestMcpServer();
            const facet = new McpFacet(
    ...
    test(
        "seeds and snapshots the registration store with defensive byte copies",
        {
            tags: "p1"
        },
        () => {
...
describe("MCP normative discovery", () => {
    test(
        "[P11-MCP-SCHEMA-BOUNDARY] validates discovered schemas before projecting Operations",
        { tags: "p1" },
        () => {
    ...
    test(
        "[P11-MCP-REVISION] enforces the edition's exact protocol revision",
        { tags: "p1" },
        () => {
    ...
    test(
        "[P11-MCP-IMPACT-ANNOTATION] [P11-MCP-IMPACT-DEFAULT-REMOTE] [P11-MCP-IMPACT-DEFAULT-LOCAL] maps only exact impact metadata and defaults unannotated remote/local tools",
        { tags: "p0" },
        () => {
    ...
    test(
        "[P11-MCP-IMPACT-UNKNOWN] rejects malformed or unknown impact metadata",
        { tags: "p0" },
        () => {
    ...
    test(
        "[P11-MCP-MALFORMED-SCHEMA] [P11-MCP-NO-LATE-SCHEMA] rejects malformed discovery metadata and schemas before registration",
        { tags: "p1" },
        () => {
    ...
    test(
        "rejects array and primitive tool metadata as invalid impact evidence",
        { tags: "p0" },
        () => {
    ...
    test(
        "[P11-MCP-POSITIVE-BOUNDS] [P11-MCP-PROMPT-COUNT] [P11-MCP-PROMPT-BYTES] enforces positive finite normative prompt item and canonical byte bounds",
        { tags: "p1" },
        () => {
            for (const bounds of [
                [0, 1],
    ...
    test(
        "[P11-MCP-SCHEMA-BOUNDARY] [P11-MCP-RESOURCES] [P11-MCP-PROMPTS] preserves validated schemas and projects resources and prompts",
        { tags: "p1" },
        () => {
            ...
            expect(discovered.operations[1]?.impact).toBe("externalSend");
    ...
    test(
        "accepts prompt sets exactly at the declared item and byte bounds",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "rejects non-integer discovery prompt bounds with the backend's own error",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "projects resources behind a closed empty input schema and omits empty prompt contributions",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "wraps malformed member primitives as the canonical malformed-document error",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "defaults unannotated metadata-bearing tools by locality and rejects malformed impact metadata",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "freezes registration documents deeply including null-bearing metadata",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "names the persisted digest when decoding a corrupt registration record",
        {
            tags: "p2"
        },
        () => {
...
describe("MCP wire codecs and error identity", () => {
    test(
        "decodes empty control inputs strictly and round-trips discovery results",
        {
            tags: "p1"
        },
        () => {
    ...
    test(
        "maps missing-operation errors to operation.missing and the rest to invalid input",
        {
            tags: "p0"
        },
        () => {
