...
describe("Agent and Run records", () => {
    ...
    it(
        ...
        () => {
            ...
            expect(otherEnvironment.environment.revision.equals(value.environment.revision)).toBe(
                true
            );
    ...
    it(
        "rejects empty, reordered, and duplicate Run configuration histories",
        { tags: "p1" },
        () => {
    ...
    it(
        "[C13-RUN-PLACEMENT-SNAPSHOT] binds placement snapshots to one Turn and one pin set",
        { tags: "p1" },
        () => {
    ...
    it(
        "enforces the closed writer matrix and complete control proposal binding",
        { tags: "p0" },
        () => {
...
describe("Turn lifecycle", () => {
    it(
        "[C13-TURN-LIFECYCLE] enforces claim, renewal, suspension, resume, completion, and terminal rejection",
        { tags: "p0" },
        () => {
            ...
            const resumedHolder = new PrincipalRef(
                ids.holder.tenantId,
                new PrincipalId("principal-2")
            );
    ...
    it(
        "[C13-TURN-NO-RETRY-EXPORT] keeps checkpoint and cancellation inbox records distinct",
        { tags: "p1" },
        () => {
...
describe("memory Run runtime", () => {
    it(
        "[C13-RUN-GRAPH-ARITY] atomically creates and restores a canonical Run graph",
        { tags: "p0" },
        () => {
            const first = harness();
            ...
            expect(restored.runtime.effectiveCommit(ids.run, ids.branch).equals(ids.root)).toBe(
                true
            );
        }
    );
    ...
    it(
        "[C13-RUN-PINS-SOURCES] rejects migration ahead of an admitted old-pin Turn",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        "[C13-TURN-NO-RETRY-RECORD] exposes no Turn retry runtime or record field",
        { tags: "p0" },
        () => {
            const value = harness();
            const placement = new TurnPlacementSnapshot(ids.turn, pins(), []);
    ...
    it(
        "creates one attenuated child through lease-bound atomic spawn genesis",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        "[C13-RUN-EQUAL-PIN-MERGE] accepts only ordered equal-pin current heads for binary merge",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        "[C13-RUN-UNDO-REDO] appends undo selection without rewinding ancestry",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
            ...
            expect(
                value.repository.transaction((tx) =>
                    value.repository.isAncestor(tx, ids.root, undo.id)
                )
            ).toBe(true);
        }
    );
    ...
    it(
        "rolls back incomplete genesis and rejects unresolved source revisions",
        { tags: "p0" },
        () => {
            const value = harness();
    ...
    it(
        "[C13-TURN-NO-RETRY-RUNTIME] cancels an unheld queued Turn without creating a result commit",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        ...
        () => {
            ...
            expect(() => terminal.recordConfiguration(digest("f"))).toThrow(
    ...
    it(
        "[C13-RUN-TERMINAL-OBLIGATIONS] derives settlement from every captured obligation",
        { tags: "p0" },
        () => {
            const value = harness();
