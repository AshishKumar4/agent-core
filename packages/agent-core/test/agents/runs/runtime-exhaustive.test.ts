...
describe("RunRuntime rejection matrix", () => {
    ...
    it(
        "[C13-RUN-PARENT-PIN-INHERITANCE] rejects migration evidence whose from pins do not match the durable parent",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        "[C13-TURN-NO-RETRY-PROTOCOL] has no retry transition and rejects a wrong effective Run",
        { tags: "p0" },
        () => {
    ...
    it(
        "[C13-RUN-MIGRATED-TURN-REJECTION] rejects duplicate commits, stale parents, pin changes, and foreign Turn writers",
        { tags: "p0" },
        () => {
            const value = seedRunningTurn();
    ...
    it(
        "[C13-ADV-NONBINARY-MERGE] rejects missing merge sources and invalid picked content",
        { tags: "p0" },
        () => {
            const value = harness();
            value.runtime.createRun(genesis());
    ...
    it(
        ...
        () => {
            ...
            // Two parents naming one commit is a unary merge wearing binary shape. It is not a
            // value either: the record carries the ordered pair rather than a length.
            expect(() => new RunCommit(mergeInit([ids.root, ids.root]))).toThrow(
                /two distinct commits/
            );
            // The wire edge supplies the parent list directly, so both bounds hold on decode
            // rather than only on the constructor a remote host never calls.
            const second = new RunCommitId("second-parent");
            const binary = new RunCommit(mergeInit([ids.root, second]));
            const octopus = mutableData(binary.toData());
            octopus["parents"] = [ids.root.value, second.value, "third-parent"];
            expect(() => RunCommit.fromData(octopus)).toThrow(/Merge commit fields/);
            const doubled = mutableData(binary.toData());
            doubled["parents"] = [ids.root.value, ids.root.value];
            expect(() => RunCommit.fromData(doubled)).toThrow(/two distinct commits/);
        }
    );
});
...
describe("Turn and terminalization rejection matrix", () => {
    it(
        "rejects ordinary inbox sequence and suspend/complete mismatches with turn.invalid-state",
        { tags: "p1" },
        () => {
            const value = seedRunningTurn();
    ...
    it(
        "rejects terminalization with mismatched result and sibling cancellation evidence",
        { tags: "p0" },
        () => {
            const value = seedRunningTurn();
    ...
    it(
        "[C13-RUN-TERMINAL-SIBLINGS] atomically fences queued, running, and suspended siblings with durable evidence",
        { tags: "p0" },
        () => {
            const value = seedRunningTurn();
            ...
            expect(
                value.repository
                    ...
                    .filter(
                        (commit) =>
                            commit.kind === "result" && !commit.subjectTurn?.equals(ids.turn)
                    )
            ).toEqual([]);
    ...
    it(
        "[C13-ADV-ADMITTED-SIBLING] rejects missing and malformed held sibling cancellation evidence",
        { tags: "p0" },
        () => {
    ...
    it(
        "rejects unsuccessful control and mismatched fence evidence atomically",
        { tags: "p0" },
        () => {
    ...
    it(
        "rolls back sibling fences, records, and registry close when the terminal commit fails",
        { tags: "p0" },
        () => {
            const value = seedRunningTurn();
