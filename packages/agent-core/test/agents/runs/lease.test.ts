...
import { seedRunningTurn } from "./fixture";

const turn = new TurnId("turn-lease-test");
const otherTurn = new TurnId("turn-lease-other");
const holder = new PrincipalId("principal-lease-holder");
const otherHolder = new PrincipalId("principal-lease-other");
const at = (milliseconds: number): Date => new Date(milliseconds);

function token(tokenTurn: TurnId = turn, tokenHolder: PrincipalId = holder, epoch = 1): LeaseToken {
...
describe("TurnLease", () => {
    ...
    test("[turn-lease] round-trips held and fenced leases through the canonical codec", () => {
        const held = TurnLease.unclaimed(turn).claim(holder, at(1), at(10));
    ...
    test("rejects malformed, future-major, and invalid lease values", () => {
        const malformed = encodeCanonicalJson({
            kind: "turn-lease",
            version: { major: 1, minor: 0 },
            payload: { turn: turn.value, holder: holder.value, epoch: "1", expiresAt: 10 }
        });
        ...
        expect(() => TurnLease.decode(unknownMajor)).toThrow(
            new AgentCoreError("codec.unknown-major", "Unsupported turn-lease codec major 2")
        );
