...
describe("closed commit writer matrix", () => {
    ...
    it(
        "[C13-WRITER-POST-FENCE-EVIDENCE] rejects every delivery and control evidence mismatch",
        { tags: "p0" },
        () => {
            const value = harness();
            expectCode(
    ...
    it(
        "[C13-ADV-UNAUTHORIZED-WRITER] rejects substitution between SystemCause and returned evidence IDs",
        { tags: "p0" },
        () => {
            const value = harness();
    ...
    it(
        "[C13-WRITER-SYNTHESIS] requires exact synthesis Run, token, and content evidence",
        { tags: "p0" },
        () => {
            const variants = [
                undefined,
            ...
            for (const [index, evidence] of variants.entries()) {
                ...
                if (evidence !== undefined)
                    value.evidence.syntheses.set("synthesis-receipt", evidence);
                expectCode(
                    () =>
                        value.repository.transaction((tx) =>
