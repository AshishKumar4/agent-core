...
describe("SQLite content row validation", () => {
    test(
        "rejects every malformed selected blob column as operational corruption",
        { tags: "p1" },
        async () => {
    ...
    test(
        "returns exact stat shape with and without media and rejects malformed media",
        { tags: "p1" },
        async () => {
            const database = new InterceptingSqlite();
            const store = new SqliteContentStore(database);
    ...
    test(
        "rolls back a blob when post-insert verification cannot observe it",
        { tags: "p0" },
        async () => {
            const database = new InterceptingSqlite();
            const store = new SqliteContentStore(database);
    ...
    test(
        "rejects incompatible pre-existing content schema without partial writes",
        { tags: "p1" },
        async () => {
...
describe("SQLite retention row validation", () => {
    ...
    test(
        "propagates an active SQLite driver TypeError from the binding read",
        { tags: "p1" },
        () => {
            const database = new InterceptingSqlite();
            const owner = contentOwner();
    ...
    test(
        "rejects every owner-edge metadata mismatch and selected column type",
        { tags: "p1" },
        async () => {
    ...
    test(
        "rejects every relation ownership mismatch and malformed nullable timestamp",
        { tags: "p1" },
        async () => {
    ...
    test(
        "rejects every transient lease metadata mismatch and selected column type",
        { tags: "p1" },
        async () => {
    ...
    test(
        "fails closed for missing related blobs, relations, and invalid unowned deadlines",
        { tags: "p0" },
        async () => {
    ...
    test(
        "rejects immutable lease collision, missing bytes, foreign bindings, and bound owner reuse",
        { tags: "p0" },
        async () => {
            const database = new TestSqlite();
            const owner = contentOwner();
            ...
            expectAgentCoreError(
                () =>
                    store.transient(
                        owner.tenant,
                        new ActorRef("workspace", new ActorId("foreign"))
                    ),
                "protocol.invalid-state"
            );
        }
    ...
    test(
        "rolls back edge, lease, close, and collection when SQLite faults after mutation",
        { tags: "p0" },
        async () => {
    ...
    test(
        "rolls back a transient lease whose inserted content cannot be verified",
        { tags: "p0" },
        async () => {
            const database = new InterceptingSqlite();
            const owner = contentOwner();
    ...
    test(
        "uses the default observation clock for transient acquisition",
        { tags: "p1" },
        async () => {
            const database = new TestSqlite();
            const owner = contentOwner();
...
describe("SQLite content restart", () => {
    test(
        "restores blob, owner, relation, and lease state after a file-backed restart",
        { tags: "p0" },
        async () => {
            ...
            try {
                ...
                const restartedAccess = restarted.transient(owner.tenant, owner.actor, () =>
                    at(30)
                );
