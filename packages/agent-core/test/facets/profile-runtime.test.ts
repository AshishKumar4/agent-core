...
function isString(value: FacetData | undefined): value is string {
    return typeof value === "string";
}

function decodeExampleInput(data: FacetData): ExampleInput {
    if (!isFacetDataMap(data) || !isString(data["value"])) {
        throw new TypeError("Expected example profile input");
    }
...
const contract = new ProfileOperationContract(
    "example",
    descriptor,
    profileWireCodec<ExampleInput>((input) => ({ value: input.value }), decodeExampleInput),
...
describe("W3 profile runtime", () => {
    test(
        "keeps profile host adapters private and uses canonical Operation execution",
        { tags: "p1" },
        async () => {
            ...
            await expect(
                // @ts-expect-error Runtime callers can supply an invalid profile input.
                runtime.invoke(contract, { value: 1 }, () => "unused")
            ).rejects.toMatchObject({ code: "operation.invalid-input", detailCode: "wire.input" });
        }
    );

    test(
        "models direct contexts without attempts and rejects partial attempt identity",
        { tags: "p0" },
        () => {
    ...
    test(
        "rejects mismatched profile contracts and non-exact host identifier classes",
        { tags: "p1" },
        () => {
            expect(
                () =>
            ...
            expect(() => voidProfileWireCodec.decode("not-null")).toThrowError(
    ...
    test(
        "fails closed on protected result substitution and wire codec failures",
        { tags: "p0" },
        async () => {
            ...
            await expect(
                // @ts-expect-error Runtime callers can supply an invalid profile input.
                template.invoke(contract, { value: 1 }, () => "unused")
            ).rejects.toMatchObject({
                code: "operation.invalid-input",
                detailCode: "wire.input",
            ...
            await expect(
                // @ts-expect-error Runtime handlers can return an invalid profile output.
                template.invoke(contract, { value: "input" }, () => 7)
            ).rejects.toMatchObject({
                code: "operation.invalid-output",
                detailCode: "wire.output",
    ...
    test(
        "adapts descriptors to distinct runtime Operations and Surfaces",
        { tags: "p1" },
        async () => {
            ...
            await expect(
                surface.render(operationContext(false), { rendered: true })
            ).resolves.toEqual({
                rendered: true
            });
    ...
    test(
        "composes standard profile manifests without exporting profile implementations",
        { tags: "p1" },
        () => {
    ...
    test(
        "coalesces lifecycle transitions, honors abort, and rejects duplicate runtime declarations",
        { tags: "p1" },
        async () => {
...
function operationContext(attempted: boolean): OperationContext {
    ...
    if (!attempted) return Object.freeze(context);
...
function deferred<Value>() {
    let accept: ((value: Value) => void) | undefined;
