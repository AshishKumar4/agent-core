...
describe("MemoryWorkspaceSlotStore snapshots", () => {
    ...
    test(
        "rejects malformed snapshots, immutable origins, and out-of-transaction access with typed errors",
        { tags: "p0" },
        async () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
    ...
    test(
        "validates every committed map edge and deterministic snapshot ordering",
        { tags: "p1" },
        async () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
    ...
    test(
        "rejects cross-workspace restore, non-Promise thenables, and duplicate draft origins",
        { tags: "p0" },
        () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
