...
describe("MemoryWorkspaceSlotStore snapshots", () => {
    test(
        "[C13-FACET-CONTRIBUTION-MATERIALIZATION] supports idempotent high-level install and contribution operations",
        { tags: "p1" },
        () => {
    ...
    test(
        "[C13-FACET-SLOT-VISIBILITY] binds SlotCatalog to an authenticated Workspace viewer and filters every entry",
        { tags: "p0" },
        async () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
    ...
    test(
        "[C13-INTERCEPTOR-THROW-BLOCK] rejects malformed snapshots, immutable origins, and out-of-transaction access with typed errors",
        { tags: "p0" },
        async () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
    ...
    test(
        "[C13-INTERCEPTOR-ORDER] validates every committed map edge and deterministic snapshot ordering",
        { tags: "p1" },
        async () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
    ...
    test(
        "[C13-INTERCEPTOR-CROSS-FACET] rejects cross-workspace restore, non-Promise thenables, and duplicate draft origins",
        { tags: "p0" },
        () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
...
describe("MemoryWorkspaceSlotStore isolation and identity", () => {
    test(
        "snapshots detach bytes from live state and order entries deterministically",
        { tags: "p1" },
        () => {
            const owner = new WorkspaceId("workspace");
            const store = new MemoryWorkspaceSlotStore(owner);
            ...
            const snapshot = store.snapshot();
            expect(
                snapshot.entries.map((bytes) => SlotEntry.decode(bytes.slice()).id.value)
    ...
    test(
        "treats byte-identical reinsertion as a no-op and equal-length divergence as immutable",
        { tags: "p1" },
        () => {
            const store = new MemoryWorkspaceSlotStore(new WorkspaceId("workspace"));
    ...
    test(
        "carries exact codes and subjects for revision, key, and origin violations",
        { tags: "p2" },
        () => {
            const store = new MemoryWorkspaceSlotStore(new WorkspaceId("workspace"));
            const declaration = slot();
    ...
    test(
        "classifies restore failures with exact codes and the malformed guard",
        { tags: "p1" },
        () => {
            const owner = new WorkspaceId("workspace");
            const declaration = slot();
    ...
    test(
        "raises typed facet.inactive and invalid-input errors from contribute",
        { tags: "p0" },
        () => {
            const store = new MemoryWorkspaceSlotStore(new WorkspaceId("workspace"));
            expectAgentCoreError(
