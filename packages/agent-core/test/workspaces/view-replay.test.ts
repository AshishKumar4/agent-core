...
describe("ViewReplayProtocol", () => {
    test(
        ...
        () => {
            ...
            expect(persistence.currentView(records, initial.surface.value)?.body).toEqual(
                next.body
            );
            expect(
        ...
        () => {
            ...
            const withoutBase = new MemoryWorkspaceRecords({
                ...
                records: snapshot.records.filter(
                    (record) =>
                        !(record.kind === "view" && record.id === `${initial.surface.value}@0`)
                )
            });
    ...
    test(
        ...
        () => {
            ...
            expect(protocol.replay(records, view.surface, Revision.initial()).kind).toBe(
                "snapshot"
            );
