...
describe("views and deltas", () => {
    ...
    test(
        ...
        () => {
            ...
            expectCodecInvalid(() =>
                ViewDelta.decode(recordBytes(ViewDelta.codec.kind, missingPatch))
            );
            expectCodecInvalid(() =>
                ViewDelta.decode(
                    recordBytes(ViewDelta.codec.kind, { ...deltaPayload, unknown: true })
                )
            );
            expectCodecInvalid(() =>
            ...
            expectCodecInvalid(() =>
                ViewDelta.decode(recordBytes("workspace.other", deltaPayload))
            );
            expect(() =>
                ViewDelta.decode(
