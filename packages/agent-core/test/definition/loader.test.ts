...
describe("Blueprint loader byte custody and placement selection", () => {
    ...
    test(
        ...
        async () => {
            ...
            const loading = loader.load(blueprint(fixture.release, { enabled: true }));
            await expect(loading).rejects.toBeInstanceOf(TypeError);
            await expect(loading).rejects.toThrow("correspondence failed");
            expect(disposed).toEqual(["./main.js", "./dependency.js"]);
        }
    );
    ...
    test("captures the first disposal failure as the thrown error", { tags: "p1" }, async () => {
        ...
        await expect(disposal).rejects.toBeInstanceOf(TypeError);
