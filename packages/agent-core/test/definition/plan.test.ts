...
describe("materialization planning", () => {
    ...
    test(
        ...
        () => {
            ...
            expect(
                Object.isFrozen((projection.desired as { placement: JsonValue }).placement)
            ).toBe(true);
            expect(
    ...
    test(
        "[C13-POLICY-EPOCH-RECHECK] rechecks supported kinds while assembling Actor and materialization plans",
        { tags: "p1" },
        () => {
            const origin = managedOrigin();
            const actor = new ActorRef("tenant", new ActorId("tenant-a"));
...
describe("materialization planning mutation boundaries", () => {
    ...
    test("labels every planning payload subject in decode errors", { tags: "p2" }, () => {
        ...
        expect(() => ActorPlan.fromData({ ...actorPlanData, origin: undefined } as never)).toThrow(
            /Actor plan origin is required/
        );
        expect(() =>
