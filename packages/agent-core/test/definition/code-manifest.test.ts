...
describe("PackageCodeManifest", () => {
            ...
            expect(
                () =>
                    new PackageCodeManifest({
                        ...
                        modules: [module("./main.js", "main")],
                        entrypoints: forged<
                            readonly [PackageCodeEntrypoint, ...PackageCodeEntrypoint[]]
                        >([])
                    })
    ...
    test("rejects noncanonical media hints in every malformed shape", { tags: "p1" }, () => {
        ...
        expect(() => build(forged<MediaHint>(null))).toThrow(
        ...
        const trapped = new Proxy(new MediaHint("text/plain"), {
            getOwnPropertyDescriptor() {
                throw new RangeError("media descriptor trap");
            }
        });
        expect(() => build(trapped)).toThrow(/Code module media must be a MediaHint/);
    });
    ...
    test("names each decoded field subject and rejects nonobject payloads", { tags: "p2" }, () => {
        ...
        expect(() => PackageCodeManifest.fromData({ ...recordData(manifest), digest: 7 })).toThrow(
