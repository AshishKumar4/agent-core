...
describe("CommandRuntime installation", () => {
    ...
    test("refuses to hand an installed Command to a different contributor in the same scope", { tags: "p0" }, () => {
        const runtime = new CommandRuntime();
...
describe("CommandRuntime mapping validation", () => {
    ...
    test("resolves boolean destination sub-schemas instead of reporting them absent", { tags: "p1" }, () => {
        expect(() =>
            new CommandRuntime().install(
                installation({
                    input: new JsonSchema({
    ...
    test("treats an argument sub-schema of false as compatible with any destination", { tags: "p1" }, () => {
        expect(() =>
            new CommandRuntime().install(
                installation({
                    arguments: new JsonSchema({
    ...
    test("requires an unconstrained argument sub-schema of true to equal its destination", { tags: "p1" }, () => {
        expectCompatible(true, true);
        expectIncompatible(true, {});
        expectIncompatible(true, { type: "number" });
    });

    test("resolves numeric destination segments past prefixItems, through items and additionalProperties", { tags: "p1" }, () => {
        const runtime = new CommandRuntime();
        ...
        expectInstallError(
            installation({ input: arrayInput, mapping: moves(["/tuple/1", { from: "/value" }]) }),
            "operation.invalid-input",
    ...
    test("compares argument and destination schemas by type, const, and enum", { tags: "p1" }, () => {
    ...
    test("checks literal moves against boolean and byte-exact destination schemas", { tags: "p1" }, () => {
    ...
    test("escapes tilde and slash property names on both sides of the required-coverage check", { tags: "p1" }, () => {
        expect(() =>
            new CommandRuntime().install(
                installation({
                    input: new JsonSchema({
    ...
    test("covers a boolean Operation input document only when it admits values", { tags: "p1" }, () => {
        expect(() =>
            new CommandRuntime().install(
...
describe("CommandRuntime binding and invocation", () => {
    ...
    test("rejects malformed, boundary, and multi-digit array indexes when binding", { tags: "p1" }, () => {
        const runtime = new CommandRuntime();
        const arraySchema = new JsonSchema({
            type: "object",
            required: ["items"],
        ...
        const read = (pointer: string, items: readonly number[]): FacetData =>
            runtime.bind(
                makeCommand({ arguments: arraySchema, mapping: moves(["/value", { from: pointer }]) }),
                { items: [...items] }
            );
