...
describe("SQLite package store exact failure and projection behavior", () => {
    test(
        "release conflicts carry the exact immutable error code and message",
        { tags: "p1" },
        () => {
    ...
    test("list fails closed on duplicate stored release keys", { tags: "p1" }, () => {
        ...
        database.tamper = (statement, rows) =>
            statement.includes("FROM definition_package_releases") && statement.includes("ORDER BY")
                ? [...rows, ...rows]
                : rows;
    ...
    test(
        "list restores canonical key order from decoded releases, not raw row order",
        { tags: "p1" },
        () => {
            const database = new RowTamperSqlite();
            const store = new SqlitePackageStore(database);
            ...
            const keys = store
                .list()
                .map((release) => `${release.id.value}@${release.version.toString()}`);
            expect(keys).toEqual(["a@1.0.0", "a@2.0.0", "a@3.0.0", "b@1.0.0"]);
        }
    );

    test(
        "getSnapshot returns undefined for a missing digest and restores revision zero exactly",
        { tags: "p1" },
        () => {
            const store = new SqlitePackageStore(new TestSqlite());
    ...
    test(
        "snapshot rows swapped across digests fail the expected-digest projection",
        { tags: "p1" },
        () => {
            const database = new RowTamperSqlite();
            const store = new SqlitePackageStore(database);
    ...
    test(
        "release rows swapped across keys fail the expected-id and expected-version projections",
        { tags: "p1" },
        () => {
            const database = new RowTamperSqlite();
            const store = new SqlitePackageStore(database);
    ...
    test(
        "malformed stored snapshot projections fail closed with exact column messages",
        { tags: "p1" },
        () => {
            const database = new RowTamperSqlite();
            const store = new SqlitePackageStore(database);
    ...
    test(
        "a stored snapshot whose bytes differ under one digest is immutable",
        { tags: "p0" },
        () => {
