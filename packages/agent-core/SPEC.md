...
Binding resolution is keyed on the name, so the name's form is fixed rather than free. A
`BindingName` MUST match exactly the canonical segment form §1.4 fixes for a `FacetRef`'s
segments, `^[a-z][a-z0-9]*(?:[.-][a-z0-9]+)*$`. An empty, blank, uppercase, or otherwise
noncanonical name is refused where it is written — a refused `bind`, a refused manifest
`BindingRequirement` (§4.1), a Blueprint rejected at validation (§9.2) — and MUST NOT be
normalized, case-folded, or renamed into an admissible one. Fixing the form decides
admissibility from the name alone, before any package code loads, which is what makes it a
validation a host reads rather than a lookup a host attempts: a name no Binding may carry
never first appears as a failure at the call that used it. One form also leaves nothing for
a consumer of the name to normalize, so two distinct BindingNames cannot fold to one key
under case-insensitive comparison or separator substitution, and a name therefore cannot
reach the Grant another name was bound to. The form constrains the key of a requirement and
never the identity of a provider, which stays the exact `FacetRef` in the exact protection
domain §4.1's dependency order resolves. This maps to
...
The explicitly passed Bindings are also the whole of what the isolate's names reach. The
capability namespace loaded code addresses holds exactly those Bindings, one entry each
under its `BindingName`, fixed when the isolate is built and acquiring no entry afterwards;
resolution over it is closed. A name the passed set does not contain MUST resolve to
nothing at all — not to a value the hosting language's environment supplies, not to a
built-in, and not to a property inherited by whatever structure a host assembled the
namespace from. A host that answered an unpassed name from anything ambient would restore
by name the authority §1.5 removes by placement, and would invert §3.4's rule that
identifiers alone confer nothing: there an identifier confers nothing because resolution
denies it, here it would confer something because resolution never ran. Closure is what
...
resolver, and every call it carries is the ordinary Invocation §7.2 tiers. This maps to
...
is what puts the route in exactly one Facet's §4.1 withdrawal set. A Subscription a caller
created directly is nobody's contribution. It carries no attribution at all, absent rather
than null or a placeholder contributor, and no withdrawal retires it. A later revision
MUST NOT add, drop, or rewrite the attribution, and the store that holds Subscriptions is
where that refusal happens, because a revision is written nowhere else. Both directions
are authority questions. A route that could acquire attribution would launder a caller's
own Subscription into a Facet's records and into that Facet's withdrawal set. A route that
could shed it would outlive the withdrawal of the Facet that contributed it and keep
routing Events through its `AuthoritySource` Binding. This maps to
...
- **C13-AUTH-BINDING-NAME-CANONICAL** A `BindingName` matches the §1.4 canonical segment form, and a noncanonical name is refused where it is written rather than normalized, case-folded, or renamed.
...
- **C13-AUTH-ISOLATE-NAMESPACE-CLOSED** An isolate's capability namespace holds exactly the explicitly passed Bindings and resolution over it is closed, so an unpassed name resolves to nothing ambient, inherited, or renamed.
...
- **C13-SUBSCRIPTION-ATTRIBUTION-FIXED** A Subscription a Facet's contribution materialized carries that contribution's `FacetRef` and `PackagePin` and is retired by exactly that Facet's withdrawal; one a caller created directly carries no attribution, encoded by absence, and no withdrawal retires it; and no later revision adds, drops, or rewrites it.
...
| Interceptors | `NC-INTERCEPTORS` | §4.4 runtime candidate discovery, durable trace persistence, transactional replay lookup, new-pass InvocationId allocation, and the `prompt.assemble`, `input.submitted`, and `turn.step` cut points are not modeled; the operation cut points are claimed under `AC-INTERCEPTOR-001`, whose ordering claim is the banded `(mode, priority, facetId, interceptorId)` key over the schedule the host supplies, and whose gate-fidelity claim is that a `gate` returning a different value is refused rather than applied |
