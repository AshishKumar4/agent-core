...
| Placement, trust, and exact-Turn leases | `AC-PLACEMENT-001`, `AC-TRUST-001`, `AC-LEASE-001` | pure four-set selection, the §7.2 tier floor including both of its session exceptions — the Turn-owned session direct-execute exception and the own-filesystem `mutate` exception, which the extracted `Facets` floor and the kernel decided first and which the abstract model plane now joins them on, so all three planes decide the floor identically, one source-tier rejection property, listed LeaseStep consequences over supplied inputs, and an executable step function proven sound and complete for the lease relation (the differential-testing oracle); no complete lifecycle claim |
