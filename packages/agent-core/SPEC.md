...
| Integrated admission and settlement | `AC-COMPOSED-001` | designated direct admission, ordinary Actor-local mediated admission, and an abstract distributed mediated-permit LTS. The target first durably records an immutable request; the Tenant issues from only its authority state and the authenticated request payload; typed messages cross a lossy/duplicating/reordering transport; and the target authenticates and consumes against the exact request, volatile authentication, fence, time, claim, reservation, lease, route, and audit state without reading issuer storage. A reachability invariant connects transported, authenticated, and consumed permits to historical Tenant issuance. Actor-local attempts remain legal and do not require a cross-Actor permit. Live authority administration is deliberately absent until a capability-mediated administration path is modeled; raw `AuthorityStep` is not admitted as a runtime transition. The abstract permit binds the modeled PreparedInvocation, claim, reservation, binding generation, fence, actor, nonce, and time fields, not every concrete §10.3 wire field. Settlement retains its constructive exact-obligation witness; no concrete transaction or refinement claim |
