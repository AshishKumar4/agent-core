...
def Settled (state : SystemState) (run : RunId) : Prop :=
  ...
  (∀ snapshot, state.graph.terminalSnapshots run = some snapshot →
    ∀ obligation, obligation ∈ snapshot.obligations → ObligationDischarged state run obligation) ∧
  ¬ state.graph.conflicts run

theorem item_obligation_uses_exact_audit {state run invocation index key}
    (discharged : ObligationDischarged state run (.item invocation index key)) :
...
theorem settled_has_coherent_snapshot_and_exact_obligations {state run}
    ...
    (∃ snapshot, state.graph.terminalSnapshots run = some snapshot ∧
      snapshot.run = run ∧ TerminalSnapshotCoherent state.graph snapshot) ∧
    (∀ snapshot, state.graph.terminalSnapshots run = some snapshot →
      ∀ obligation, obligation ∈ snapshot.obligations → ObligationDischarged state run obligation) := by
...
/-- A captured acceptance criterion that holds no current satisfying verdict keeps the Run
    unfinished: it is snapshotted into the SettlementObligation and the Run is not Settled
    while it stands, exactly as an outstanding Approval would be (§5.2,
