...
#guard Adversarial.cases.length == 54
...
#guard Adversarial.allScrambles.length == 552
...
#guard (lexicon.filter (fun entry => entry.caveats.contains .typeAsCommonNoun)).length == 4
#guard (lexicon.filter (fun entry =>
