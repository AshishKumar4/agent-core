...
#guard Adversarial.cases.length == 143
...
#guard Adversarial.allScrambles.length == 1383
...
#guard (lexicon.filter (fun entry => entry.caveats.contains .typeAsCommonNoun)).length == 8
#guard (lexicon.filter (fun entry =>
