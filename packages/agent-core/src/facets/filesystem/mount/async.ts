...
export class MountedFileSystem extends FileSystem {
    readonly #mounts: MountTable<FileSystem>;
    #revision = 0;

    public constructor(
        private readonly root: FileSystem,
        mounts: readonly Mount<FileSystem>[]
    ) {
        super();
    ...
    public async stat(
        ...
        return route.entry(await route.target.stat(context, route.path));
    }
    ...
    public async write(
        ...
        const route = this.#mounts.route(path, this.root);
        const receipt = await route.target.write(
            context,
            route.path,
            content,
            mode,
    ...
    public async list(
        ...
        if (!path.root) {
            const route = this.#mounts.route(path, this.root);
            const current = await route.target.list(
                context,
                route.path,
                position,
                limit
            ...
            return new FilePage(
                current.entries.map(entry => route.entry(entry)),
                current.continuation
            );
        }

        const entries = await this.rootEntries(context);
    ...
    public async makeDirectory(
        ...
        const route = this.#mounts.route(path, this.root);
        const receipt = await route.target.makeDirectory(
            context,
            route.path,
            mode
        );
    ...
    public async remove(
        ...
        const route = this.#mounts.route(path, this.root);
        const receipt = await route.target.remove(context, route.path, mode);
        this.changed();
        return receipt;
    }
    ...
    public async move(
        ...
        const receipt = await from.target.move(
            context,
            from.path,
            to.path,
            mode
    ...
    public async flush(context: OperationContext): Promise<MutationReceipt> {
        for (const target of this.#mounts.targets(this.root)) {
            const receipt = await target.flush(context);
    ...
    private async rootEntries(
        ...
        while (true) {
            const current = await this.root.list(
                context,
                FilePath.root(),
                position,
                256
