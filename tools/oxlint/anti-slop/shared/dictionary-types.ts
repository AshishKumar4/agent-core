...
type TypeDeclarationScope =
    ESTree.BlockStatement | ESTree.Program | ESTree.SwitchCase | ESTree.TSModuleBlock;
...
const lexicalEnvironmentCache = new WeakMap<TypeDeclarationScope, TypeEnvironment>();
...
function scopeStatements(scope: TypeDeclarationScope): readonly ESTree.Statement[] {
    if (
        scope.type === "Program" ||
...
function isTypeDeclarationScope(node: ESTree.Node): node is TypeDeclarationScope {
    return (
        node.type === "Program" ||
...
function addScopeDeclarations(
    scope: TypeDeclarationScope,
    ...
    const localAliases = new Map<string, ESTree.TSTypeAliasDeclaration>();
    const localInterfaces = new Map<string, ESTree.TSInterfaceDeclaration[]>();
    ...
    for (const statement of scopeStatements(scope)) {
        ...
        if (declaration?.type === "TSTypeAliasDeclaration") {
            if (!localAliases.has(declaration.id.name)) {
    ...
    for (const [name, alias] of localAliases) aliases.set(name, alias);
    for (const [name, declarations] of localInterfaces) interfaces.set(name, declarations);
}

function environmentForScopes(scopes: readonly TypeDeclarationScope[]): TypeEnvironment {
    const aliases = new Map<string, ESTree.TSTypeAliasDeclaration>();
    const interfaces = new Map<string, ESTree.TSInterfaceDeclaration[]>();
    const shadowedBuiltIns = new Set<string>();
    for (const scope of scopes) addScopeDeclarations(scope, aliases, interfaces, shadowedBuiltIns);

    return { aliases, interfaces, shadowedBuiltIns };
}

export function createTypeEnvironment(program: ESTree.Program): TypeEnvironment {
    return environmentForScopes([program]);
}

export function createLexicalTypeEnvironment(node: ESTree.Node): TypeEnvironment {
    const scopes: TypeDeclarationScope[] = [];
    let current: ESTree.Node | null = node;
    while (current !== null) {
        if (isTypeDeclarationScope(current)) scopes.push(current);
        current = current.parent;
    }
    const cacheKey = scopes[0];
    if (cacheKey === undefined) return environmentForScopes([]);
    const cached = lexicalEnvironmentCache.get(cacheKey);
    if (cached !== undefined) return cached;
    const environment = environmentForScopes(scopes.reverse());
    lexicalEnvironmentCache.set(cacheKey, environment);
    return environment;
}
