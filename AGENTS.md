...
**Live Substrate Evidence:** Consent-gated substrate conformance atoms are verified only against the real deployed substrate; committed live evidence must be fully passing, and hash-bound to the exact current sources wherever any row still claims `verified` from it, never substituted with a local emulation. A source that drifted after the archived run is a lie under a verified claim and fails closed, and a re-run this tree is waiting for once every row citing it has retreated below `verified` — in which case the drifted sources and the waiting rows are enumerated as pending. The retreat is the refusal: an atom the archive no longer covers is held below `verified` and, where the operator's consent is what resolves it, `external-gated` within the atoms that consent names. Re-pinning a fingerprint by hand ratifies a deployment that did not happen, and neither the pending state nor the final conformance stage admits it.

**Generated Compiler Artifacts:** A generated TSLean package is byte-bound to its manifest: its bytes come only from the proved compiler, the consumer gate verifies every digest and certificate offline, and editing or reformatting the generated tree is a defect repaired by regeneration, never by hand.
...
**Normative Coverage:** Every rule of the SPEC's declared normative sections that uses a declared RFC 2119 keyword is anchored to a conformance atom in the same block, list item, or table row it is stated in; anchoring a whole numbered list at one rule binds the others to nothing. A rule no atom anchors is judged in `artifacts/quality/normative-coverage.json` by the digest of its exact prose — `exempt` where the document itself denies the clause any binding effect, `unanchored` where it names the atom the anchor is owed to — so rewording the rule reopens the judgement, and an unjudged rule is never accepted as baseline debt.

**Mutant Equivalence:** A surviving mutant is excused from the actionability ratchet only by its own entry in `artifacts/quality/mutation-equivalence.json`, never by a count-level override. Each entry names one mutant — file, enclosing symbol, mutator, replacement, and the exact source the mutator replaces — and carries the written proof that the mutation leaves observable behavior unchanged; a restated conclusion is not a proof, and one proof never covers two mutants. The claim is falsifiable in both directions: a registered mutant that any run reports killed refutes its own proof and fails the measurement by name, and an entry whose anchor no longer resolves to exactly one live mutant is stale and fails until the proof is rewritten against the code that replaced it.
